import { ProductTyping } from "@/providers/ProductsProvider";

enum feedBackEnum {
  DROPDOWN = "dropdown",
  RADIO = "radio",
  TEXT = "text",
  DATE = "date",
  CHECKBOX = "checkbox",
  COUNTRY = "country",
}

export const productsData = {
  /* -------------------------- VISA PREPS -------------------------- */
  [ProductTyping.VISA_PREPS]: {
    general: [
      {
        question: "What type of visa are you applying for?",
        type: feedBackEnum.DROPDOWN,
        helpText: "Select your visa type from the dropdown list",
        answer: [
          "Study",
          "Work",
          "Visitor",
          "Family",
          "Skilled Worker",
          "Business",
          "Other — specify",
        ],
      },
      {
        question: "What country are you applying to?",
        type: feedBackEnum.TEXT,
        helpText: "Enter the destination country name",
      },
    ],

    study: [
      {
        question: "Are you admitted to a program/institution?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Name of institution / school",
        type: feedBackEnum.TEXT,
        helpText: "Enter full institution name",
      },
      {
        question: "Program name & level",
        type: feedBackEnum.TEXT,
        helpText: "e.g. BSc Nursing, Master of Data Science",
      },
      {
        question: "Program start date",
        type: feedBackEnum.DATE,
        helpText: "Pick the date your program starts",
      },
      {
        question: "Type of offer received",
        type: feedBackEnum.RADIO,
        helpText: "Select the applicable offer type",
        answer: ["Conditional Offer", "Unconditional Offer", "Acceptance Letter"],
      },
      {
        question: "Is your tuition paid / deposit paid?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes, No, or Partially",
        answer: ["Yes", "No", "Partially"],
      },
      {
        question: "Did you receive any scholarships/financial aid?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Do you need help with proof of funds or no-collateral loan?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
    ],

    other: [
      {
        question: "Have you previously traveled abroad?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "List countries visited in last 10 years",
        type: feedBackEnum.TEXT,
        helpText: "Enter country names separated by commas",
      },
      {
        question: "Have you ever been refused a visa for any country?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Have you ever been banned from entering any country?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Have you ever been deported or removed from a country?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Do you have any pending immigration or asylum applications elsewhere?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Have you previously overstayed a visa?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
    ],

    last_batch: [
      {
        question: "Have you booked your visa interview yet?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, what is your interview date?",
        type: feedBackEnum.DATE,
        helpText: "Select the scheduled interview date",
      },
      {
        question: "Have you attended any previous visa interviews?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, what was the outcome?",
        type: feedBackEnum.RADIO,
        helpText: "Select the result of your past interview",
        answer: ["Approved", "Refused"],
      },
      {
        question: "What areas do you feel you need help with most?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select one or more areas where you need support",
        answer: ["Confidence", "Common questions", "Documents", "Mock interview", "Other"],
      },
      {
        question: "Which interview format would you like?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select your preferred interview preparation format(s)",
        answer: [
          "Mock oral interview (1:1)",
          "Group practice",
          "Recorded mock interview with feedback",
          "Document review only",
          "Q&A cheat sheet",
        ],
      },
      {
        question: "Preferred platform for the session",
        type: feedBackEnum.CHECKBOX,
        helpText: "Select the communication platform you prefer",
        answer: ["Zoom", "Google Meet", "WhatsApp call", "Phone call", "In-person if local"],
      },
      {
        question: "Have you read and agreed to Naijaspora’s Terms of Service and Privacy Policy?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to confirm consent",
        answer: ["I agree", "No"],
      },
    ],
  },

  /* -------------------------- LOAN / POF -------------------------- */
  [ProductTyping.LOAN_POF]: {
    general: [
      {
        question: "Name of school or institution",
        type: feedBackEnum.TEXT,
        helpText: "Enter the full school name",
      },
      {
        question: "Country of school / destination country",
        type: feedBackEnum.TEXT,
        helpText: "Enter the country name",
      },
      { question: "Program level", type: feedBackEnum.TEXT, helpText: "e.g., BSc, MSc, PhD" },
      {
        question: "Program name",
        type: feedBackEnum.TEXT,
        helpText: "e.g., BSc Nursing, MSc Data Science",
      },
      {
        question: "Program start date",
        type: feedBackEnum.DATE,
        helpText: "Select the program start date",
      },
      {
        question: "How much is your tuition and living cost?",
        type: feedBackEnum.TEXT,
        helpText: "Enter the total cost (₦ or foreign currency)",
      },
      {
        question: "Is your offer conditional or unconditional?",
        type: feedBackEnum.RADIO,
        helpText: "Select the offer type",
        answer: ["Conditional", "Unconditional"],
      },
      {
        question: "What type of financial help do you need?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select one or more options that apply",
        answer: [
          "Proof of funds only (for visa purpose)",
          "Tuition loan",
          "Living expense support",
          "All of the above",
          "Other (specify)",
        ],
      },
      {
        question: "Total amount needed (in either ₦ or foreign currency)",
        type: feedBackEnum.TEXT,
        helpText: "Enter the total amount required",
      },
      {
        question:
          "Do you have a guarantor or next of kin who is Permanent Resident or citizen of another country besides Nigeria (not collateral)?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question:
          "Do you authorize Naijaspora to verify your documents and share them with partner lenders or financial institutions for assessment?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to authorize",
        answer: ["I agree"],
      },
      {
        question:
          "Have you or your sponsor ever been involved in any fraudulent activity, loan default, or financial dispute?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question:
          "Do you understand that falsified documents or incomplete information may lead to disqualification?",
        type: feedBackEnum.RADIO,
        helpText: "Select I understand to confirm",
        answer: ["I understand"],
      },
      {
        question: "Have you read and agreed to Naijaspora's Terms of Service and Privacy Policy?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to continue",
        answer: ["I agree"],
      },
    ],
  },

  /* ----------------------- AGENT VERIFICATION ----------------------- */
  [ProductTyping.AGENT_VERIFICATION]: {
    general: [
      {
        question: "Full Name of the Agent or Agency",
        type: feedBackEnum.TEXT,
        helpText: "Enter the full agent or agency name",
      },
      {
        question: "Agent's Phone Number(s)",
        type: feedBackEnum.TEXT,
        helpText: "Enter phone number(s) with country code",
      },
      {
        question: "Agent's Email Address",
        type: feedBackEnum.TEXT,
        helpText: "Enter a valid email address",
      },
      {
        question: "Agent's Office Address or Location",
        type: feedBackEnum.TEXT,
        helpText: "City, State, and Country",
      },
      {
        question: "What service is the agent offering?",
        type: feedBackEnum.TEXT,
        helpText: "Describe the service offered",
      },
      {
        question: "How did you find this agent?",
        type: feedBackEnum.TEXT,
        helpText: "e.g., referral, social media, website",
      },
      {
        question: "Agent's Website or Social Media Handle",
        type: feedBackEnum.TEXT,
        helpText: "Provide link or handle",
      },
      {
        question: "Does the agent claim to be affiliated with any company, school, or embassy?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, please provide the name of the organization or company",
        type: feedBackEnum.TEXT,
        helpText: "Enter the claimed affiliated company or organization name",
      },
      {
        question:
          "Has the agent provided you with any business identification, license, or certificate?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "What service is the agent providing for you?",
        type: feedBackEnum.DROPDOWN,
        helpText: "Select one option from the list",
        answer: [
          "Visa Processing",
          "School Admission",
          "Job Placement",
          "Housing Arrangement",
          "Sponsorship",
          "Other",
        ],
      },
      {
        question: "What country or city is this service related to?",
        type: feedBackEnum.TEXT,
        helpText: "Enter the country or city",
      },
      {
        question: "Has the agent asked for any upfront payment or deposit?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, how much and in what currency?",
        type: feedBackEnum.TEXT,
        helpText: "Enter amount and currency, e.g., $500 USD",
      },
      {
        question: "What payment method was suggested by the agent?",
        type: feedBackEnum.DROPDOWN,
        helpText: "Select from the dropdown",
        answer: ["Bank Transfer", "Cryptocurrency", "Western Union", "Cash", "PayPal", "Other"],
      },
      {
        question: "Has any money already been paid?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If Yes, How Much",
        type: feedBackEnum.TEXT,
        helpText: "Enter amount and specify currency",
      },
      {
        question: "If yes, do you have any proof of payment?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Was any formal agreement, receipt, or contract signed?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question:
          "Would you like Naijaspora to contact the agent directly as part of the verification process?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Do you want your identity kept confidential during the verification?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Have you read and agreed to Naijaspora's Terms of Service and Privacy Policy?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to continue",
        answer: ["I agree"],
      },
    ],
  },

  /* --------------------- DOCUMENT VERIFICATION --------------------- */
  [ProductTyping.DOCUMENTS_VERIFICATION]: {
    general: [
      {
        question: "What type of document or certificate would you like us to verify?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all applicable document types",
        answer: [
          "Certificate of Sponsorship (COS)",
          "Admission Letter or Offer of Admission",
          "Employment Letter / Job Offer",
          "Visa or Work Permit Document",
          "Invitation Letter",
          "School Transcript / Reference Letter",
          "Passport or ID Verification",
          "Bank Statement or Proof of Funds",
          "Other (please specify)",
        ],
      },
      {
        question: "Who issued the document you want verified?",
        type: feedBackEnum.TEXT,
        helpText: "Enter the full name of the issuing institution or organization",
      },
      {
        question: "What country is the issuing institution or organization based in?",
        type: feedBackEnum.TEXT,
        helpText: "Enter the country name",
      },
      {
        question:
          "Did you receive the document directly from the organization or through an intermediary?",
        type: feedBackEnum.RADIO,
        helpText: "Select Directly or Through intermediary",
        answer: ["Directly", "Through intermediary"],
      },
      {
        question: "If through someone else, provide their full name, email, and phone number",
        type: feedBackEnum.TEXT,
        helpText: "Enter name, email, and phone number separated by commas",
      },
      {
        question: "What do you want Naijaspora to verify specifically?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all applicable verification types",
        answer: [
          "Verify if the organization or issuer is legitimate",
          "Confirm the authenticity of signatures, seals, or letterheads",
          "Confirm that the reference number exists in the institution's record",
          "Check for signs of document manipulation or forgery",
          "Verify that the COS number is valid with the UK Home Office database",
          "Confirm that the offer or admission exists in the school's record",
          "Verify employer legitimacy and job existence",
          "Other(s)",
        ],
      },
      {
        question: "How urgent is your verification request?",
        type: feedBackEnum.DROPDOWN,
        helpText: "Select the preferred turnaround time",
        answer: ["Within 24 hours", "2-3 days", "1 week"],
      },
      {
        question:
          "Do you want Naijaspora to contact the issuing organization directly on your behalf?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Should we keep your identity confidential when contacting them?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question:
          "I confirm that all documents submitted are mine or provided with the owner's consent",
        type: feedBackEnum.RADIO,
        helpText: "Select I confirm to acknowledge ownership",
        answer: ["I confirm"],
      },
    ],
  },

  /* ----------------------- SCAM ALERT / REPORT SCAM ----------------------- */
  [ProductTyping.SCAM_ALERT]: {
    general: [
      {
        question: "Are you reporting this scam for:",
        type: feedBackEnum.RADIO,
        helpText: "Select who you're reporting for",
        answer: ["Yourself", "Someone else"],
      },
      {
        question: "Where are you currently located?",
        type: feedBackEnum.RADIO,
        helpText: "Select your current location",
        answer: ["Nigeria", "Abroad"],
      },
      {
        question: "If abroad, which country?",
        type: feedBackEnum.TEXT,
        helpText: "Enter the country name",
      },
      {
        question: "Name of the person/company/agency involved (if known)",
        type: feedBackEnum.TEXT,
        helpText: "Enter the scammer's name or company",
      },
      {
        question: "Scammer's phone number(s)",
        type: feedBackEnum.TEXT,
        helpText: "Enter phone number(s)",
      },
      {
        question: "Scammer's email address (if available)",
        type: feedBackEnum.TEXT,
        helpText: "Enter email address",
      },
      {
        question: "Scammer's social media handle(s) / website (if any)",
        type: feedBackEnum.TEXT,
        helpText: "Enter social media handles or website",
      },
      {
        question: "What service did the scammer claim to offer?",
        type: feedBackEnum.TEXT,
        helpText: "e.g., job, housing, admission, sponsorship, visa processing",
      },
      {
        question: "Where is the scammer located (city/state/country)?",
        type: feedBackEnum.TEXT,
        helpText: "Enter scammer's location if known",
      },
      {
        question: "How did you find or get connected to this person/agent?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: ["Social media", "WhatsApp", "Referral", "Website", "Advertisement", "Other"],
      },
      {
        question: "When did the incident occur?",
        type: feedBackEnum.TEXT,
        helpText: "Enter date or date range",
      },
      {
        question: "Describe what happened",
        type: feedBackEnum.TEXT,
        helpText: "Provide a detailed description of the incident",
      },
      {
        question: "Did the scammer provide registration documents?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Did you make any payment?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, how much did you pay and in what currency?",
        type: feedBackEnum.TEXT,
        helpText: "Enter amount and currency",
      },
      {
        question: "How did you pay?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select payment method(s)",
        answer: [
          "Bank transfer",
          "Crypto",
          "Western Union / MoneyGram",
          "Cash",
          "PayPal",
          "Other",
        ],
      },
      {
        question: "Do you have proof of payment?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Did you lose money or personal documents?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: ["Lost money", "Lost personal documents", "Both", "Neither"],
      },
      {
        question: "What was the result of the interaction?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "The scammer disappeared",
          "Service was never delivered",
          "Ongoing conversation",
          "Other",
        ],
      },
      {
        question: "What would you like Naijaspora to do?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Verify the person/agency",
          "Help recover funds",
          "Report to authorities",
          "Publicly warn others",
          "Add scammer to blacklist database",
          "Just reporting — no action required",
          "Other",
        ],
      },
      {
        question: "Would you like Naijaspora to contact the person/agency directly?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Should we keep your identity confidential?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "How would you prefer us to contact you?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select preferred contact method(s)",
        answer: ["Email", "WhatsApp", "Phone", "No contact needed"],
      },
      {
        question: "I confirm that the information I provided is true to the best of my knowledge",
        type: feedBackEnum.RADIO,
        helpText: "Confirm to continue",
        answer: ["I agree"],
      },
      {
        question: "Have you read and agreed to Naijaspora's Terms of Service and Privacy Policy?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to continue",
        answer: ["I agree"],
      },
    ],
  },

  /* -------------------- POST VISA DENIAL SUPPORT -------------------- */
  [ProductTyping.POST_VISA_DENIAL]: {
    general: [
      {
        question: "What country did you apply to?",
        type: feedBackEnum.TEXT,
        helpText: "Enter the country name",
      },
      {
        question: "Did you get banned from applying?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "What type of visa were you applying for?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: ["Study", "Work", "Skilled Worker", "Visitor", "Family", "Business", "Other"],
      },
      {
        question: "Did an agent or consultant assist you with your application?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Is the agent an approved Naijaspora verified agent?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Did you attend a physical interview?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, where? (City/Location)",
        type: feedBackEnum.TEXT,
        helpText: "Enter interview location",
      },
      {
        question: "Was this your first visa refusal?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If no, how many times have you been denied before? (Country & year)",
        type: feedBackEnum.TEXT,
        helpText: "Enter previous denials",
      },
      {
        question: "What reason(s) were given for your denial?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Insufficient funds",
          "Weak home ties",
          "Purpose of travel unclear",
          "Missing document(s)",
          "Study mismatch",
          "Work experience mismatch",
          "Employment concerns",
          "Fraud / authenticity concerns",
          "Travel history",
          "Other",
        ],
      },
      {
        question: "Did you apply alone or with dependents?",
        type: feedBackEnum.TEXT,
        helpText: "Enter 'Alone' or number of dependents",
      },
      {
        question: "What support do you need from Naijaspora?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Review refusal letter",
          "Re-apply strategy",
          "Improve SOP / purpose letter",
          "Review financial documents",
          "Document rebuilding",
          "Interview coaching",
          "Agent / school verification",
          "Other",
        ],
      },
      {
        question: "Preferred consultation format",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select preferred format(s)",
        answer: ["Phone", "WhatsApp Call", "Zoom", "Google Meet", "Physical"],
      },
      {
        question: "I confirm that the information provided is true",
        type: feedBackEnum.RADIO,
        helpText: "Confirm to continue",
        answer: ["I agree"],
      },
      {
        question: "Have you read and agreed to Naijaspora's Terms of Service and Privacy Policy?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to continue",
        answer: ["I agree"],
      },
    ],
  },

  /* ----------------------- CONSULTATION ----------------------- */
  [ProductTyping.CONSULTATION]: {
    general: [
      {
        question: "Are you currently in Nigeria or abroad?",
        type: feedBackEnum.RADIO,
        helpText: "Select your current location",
        answer: ["Nigeria", "Abroad"],
      },
      {
        question: "If abroad, which country?",
        type: feedBackEnum.TEXT,
        helpText: "Enter country name",
      },
      {
        question: "What type of consultation do you need?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select one or more",
        answer: [
          "Study Abroad",
          "Work Abroad",
          "Immigration pathways (PR, Express Entry, Skilled Worker)",
          "Post-Visa Refusal",
          "Agent verification",
          "Proof of Funds / loans",
          "Housing / relocation aid",
          "Diaspora support",
          "Business / investment migration",
          "Other",
        ],
      },
      {
        question: "What country are you seeking guidance for?",
        type: feedBackEnum.TEXT,
        helpText: "Enter country name",
      },
      {
        question: "Briefly describe your situation (why you are booking this consultation)",
        type: feedBackEnum.TEXT,
        helpText: "Provide context about your situation",
      },
      {
        question: "What are your main questions or concerns?",
        type: feedBackEnum.TEXT,
        helpText: "List your key questions",
      },
      {
        question: "How urgent is your consultation request?",
        type: feedBackEnum.RADIO,
        helpText: "Select urgency level",
        answer: ["Very urgent", "Within 3 days", "Within a week", "No urgency"],
      },
      {
        question: "Preferred consultation style",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select preferred style(s)",
        answer: ["Video (Zoom / Google Meet)", "Voice call (WhatsApp / Phone)", "Chat only"],
      },
      {
        question: "Preferred communication method",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select preferred method(s)",
        answer: ["Email", "WhatsApp", "Phone"],
      },
      {
        question: "Preferred date/time for consultation",
        type: feedBackEnum.TEXT,
        helpText: "Enter preferred date/time",
      },
      {
        question: "Would you like your identity kept confidential during follow-up actions?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "How did you hear about Naijaspora?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: ["Social media", "Referral", "YouTube", "Website", "Other"],
      },
      {
        question: "I confirm that all information given is accurate",
        type: feedBackEnum.RADIO,
        helpText: "Confirm to continue",
        answer: ["I agree"],
      },
      {
        question: "Have you read and agreed to Naijaspora's Terms of Service and Privacy Policy?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to continue",
        answer: ["I agree"],
      },
    ],
  },

  /* ------------- POST RELOCATION / POST ARRIVAL SUPPORT ------------- */
  [ProductTyping.POST_RELOCATION]: {
    general: [
      {
        question: "Country you relocated to",
        type: feedBackEnum.TEXT,
        helpText: "Enter country name",
      },
      {
        question: "City/State of residence",
        type: feedBackEnum.TEXT,
        helpText: "Enter city and state",
      },
      {
        question: "Date of arrival (approx.)",
        type: feedBackEnum.DATE,
        helpText: "Select approximate arrival date",
      },
      {
        question: "What type of visa did you enter with?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select visa type(s)",
        answer: ["Study", "Work", "PR", "Visitor", "Dependant", "Other"],
      },
      {
        question: "Did you relocate alone or with dependants?",
        type: feedBackEnum.TEXT,
        helpText: "Enter 'Alone' or number of dependants",
      },
      {
        question: "What is your current accommodation situation?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Temporary stay (Airbnb / hotel)",
          "Living with friend/family",
          "Already secured permanent housing",
          "Need help finding housing",
          "Other",
        ],
      },
      {
        question: "Did you use an agent or organization for relocation?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, is the agent an approved/verified Naijaspora agent?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, provide agent name/contacts",
        type: feedBackEnum.TEXT,
        helpText: "Enter agent details",
      },
      {
        question: "Do you need help finding:",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: ["Part-time jobs", "Full-time jobs", "Internship", "Licensing pathway", "None"],
      },
      {
        question: "What other post-relocation services do you need help with?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Housing/Roommates",
          "Airport pickup",
          "Community connection",
          "Job search support",
          "Documentation (SIN/NIN/UCI etc.)",
          "School setup/orientation",
          "Bank account setup",
          "Healthcare registration",
          "Transportation guidance",
          "Childcare / school enrollment",
          "Other",
        ],
      },
      {
        question: "What challenges are you currently facing?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Housing difficulty",
          "Financial challenges",
          "Transportation issues",
          "Childcare issues",
          "Documentation delays",
          "Employment difficulty",
          "Feeling isolated",
          "Language barrier",
          "Scam concerns",
          "Cultural adjustment",
          "Other",
        ],
      },
      {
        question: "Preferred consultation format",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select preferred format(s)",
        answer: ["WhatsApp", "Zoom / Google Meet", "Phone call", "Email"],
      },
      {
        question: "Preferred time for consultation",
        type: feedBackEnum.TEXT,
        helpText: "Enter preferred time",
      },
      {
        question: "How did you hear about Naijaspora?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: ["Social media", "Referral", "Google", "Other"],
      },
      {
        question: "Any additional comments or details we should know?",
        type: feedBackEnum.TEXT,
        helpText: "Enter any additional information",
      },
      {
        question: "I confirm that the information submitted is accurate",
        type: feedBackEnum.RADIO,
        helpText: "Confirm to continue",
        answer: ["I agree"],
      },
      {
        question: "Have you read and agreed to Naijaspora's Terms of Service and Privacy Policy?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to continue",
        answer: ["I agree"],
      },
    ],
  },

  /* -------------------- DIASPORA PROJECT MANAGEMENT -------------------- */
  [ProductTyping.DIASPORA_PROJECT]: {
    general: [
      {
        question: "Country of residence",
        type: feedBackEnum.TEXT,
        helpText: "Enter your current country of residence",
      },
      {
        question: "Preferred communication method",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select preferred method(s)",
        answer: ["WhatsApp", "Email", "Phone call", "Other"],
      },
      {
        question: "What type of project do you want Naijaspora to manage?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Property construction / supervision",
          "Real estate purchase / land verification",
          "Family/Dependent support",
          "Small business management",
          "Renovation / home improvement",
          "Vehicle purchase / supervision",
          "Event coordination",
          "Document processing",
          "Vendor/contractor management",
          "Other",
        ],
      },
      {
        question: "Briefly describe the project",
        type: feedBackEnum.TEXT,
        helpText: "Provide a detailed description of your project",
      },
      {
        question: "What city/state is the project located in?",
        type: feedBackEnum.TEXT,
        helpText: "e.g., Lagos, Abuja, Ibadan",
      },
      {
        question: "What is the expected start date of the project?",
        type: feedBackEnum.DATE,
        helpText: "Select expected start date",
      },
      {
        question: "Is this an ongoing project or new?",
        type: feedBackEnum.RADIO,
        helpText: "Select project status",
        answer: ["New", "Ongoing"],
      },
      {
        question: "If ongoing, how far along is the project?",
        type: feedBackEnum.TEXT,
        helpText: "Describe current progress",
      },
      {
        question: "What specific tasks do you want us to handle?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Full project handling",
          "Site inspection",
          "Vendor sourcing",
          "Contractor vetting",
          "Project supervision",
          "Materials purchase",
          "Payment handling/verification",
          "Progress reporting (photo/video)",
          "Legal documentation",
          "Logistics",
          "Other",
        ],
      },
      {
        question: "Preferred update frequency",
        type: feedBackEnum.RADIO,
        helpText: "Select update frequency",
        answer: ["Daily", "Weekly", "Bi-weekly", "Monthly"],
      },
      {
        question: "Preferred update format",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select preferred format(s)",
        answer: ["WhatsApp messages", "Email", "Photos", "Videos", "Live check-ins"],
      },
      {
        question: "Do you have a representative we will collaborate with locally?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, provide name, phone, and relationship",
        type: feedBackEnum.TEXT,
        helpText: "Enter representative details",
      },
      {
        question: "Are there current contractors/vendors involved?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "If yes, provide details",
        type: feedBackEnum.TEXT,
        helpText: "Enter contractor/vendor details",
      },
      {
        question: "What deliverables do you expect?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: [
          "Written report",
          "Photo evidence",
          "Video update",
          "Cost breakdown",
          "Completion certificate",
          "Escrow service",
          "Other",
        ],
      },
      {
        question: "Timeline expectations",
        type: feedBackEnum.RADIO,
        helpText: "Select timeline preference",
        answer: ["Flexible", "Fixed"],
      },
      {
        question: "If fixed, specify date",
        type: feedBackEnum.DATE,
        helpText: "Select target completion date",
      },
      {
        question: "Would you like your identity kept confidential from contractors/vendors?",
        type: feedBackEnum.RADIO,
        helpText: "Select Yes or No",
        answer: ["Yes", "No"],
      },
      {
        question: "Are there any security concerns or special instructions?",
        type: feedBackEnum.TEXT,
        helpText: "Describe any security concerns or special instructions",
      },
      {
        question: "I confirm all information provided is accurate",
        type: feedBackEnum.RADIO,
        helpText: "Confirm to continue",
        answer: ["I agree"],
      },
      {
        question: "Have you read and agreed to Naijaspora's Terms of Service and Privacy Policy?",
        type: feedBackEnum.RADIO,
        helpText: "Select I agree to continue",
        answer: ["I agree"],
      },
      {
        question: "How did you hear about Naijaspora?",
        type: feedBackEnum.CHECKBOX,
        multiple: true,
        helpText: "Select all that apply",
        answer: ["Referral", "Social media", "Google search", "Other"],
      },
      {
        question: "Additional notes or special requests",
        type: feedBackEnum.TEXT,
        helpText: "Enter any additional notes",
      },
    ],
  },
};
